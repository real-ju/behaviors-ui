...
export default {
  ...
  methods: {
    // 由于性能原因，v-model绑定值不会在改变后重新初始化（更新视图），需要自行调用updateView()
    updateView() {
      this.$nextTick(() => {
        this.init();
    ...
    updateDayColumn(currentIndexes) {
      ...
      this.$refs.selectView.updateSingleColumn(dayIndex, datas);
    },
    init() {
      if (this.mode === "time") {
